...
class DatasiswaController extends Controller
{
    public function index(Request $request)
    {
        ...
        if (!empty($cari)) {
            $siswas = Siswa::where('nama_siswa', 'LIKE', '%' . $cari . '%')
                ->orWhere('tmpt_lhr', 'LIKE', '%' . $cari . '%')
                ->get();
        } else {
            $siswas = Siswa::all();
        }

        return view('admin.manajemen_data_siswa.index')->with([
            'siswas' => $siswas,
            'cari' => $cari
        ]);
    }
    ...
    public function update($id, Request $request)
    {
        ...
        // Periksa apakah ada file foto KK diunggah
        if ($request->hasFile('foto_kk')) {
            ...
            // Update nama file foto KK
            $validatedData['foto_kk'] = $fotokk_nama;
        }

        // Periksa apakah ada file foto akte diunggah
